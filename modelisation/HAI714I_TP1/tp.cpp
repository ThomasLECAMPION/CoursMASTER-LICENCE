...
void setUnitSphere( Mesh & o_mesh, int nX, int nY )
{
    ...
    for(int i=0; i<nX; i++) {
        for(int j=0; j<nY; j++) {
            ...
            v3 = v1 + (nY+1);
            if(i!=0) {
              o_mesh.triangles.push_back( Triangle(v1,v3,v1+1) );
            }
            if(i!=(nX-1)) {
              o_mesh.triangles.push_back( Triangle(v1+1,v3,v3+1) );
            }
        }
    }
    ...
    for(unsigned int i=0; i<o_mesh.triangles.size(); i++) {
        int s1 = o_mesh.triangles[i][0];
        int s2 = o_mesh.triangles[i][1];
        int s3 = o_mesh.triangles[i][2];

        u = o_mesh.vertices[s2] - o_mesh.vertices[s1];
        w = o_mesh.vertices[s3] - o_mesh.vertices[s1];
        ...
        o_mesh.normals.push_back( Vec3(nx,ny,nz)/n );
    }
    */
}
