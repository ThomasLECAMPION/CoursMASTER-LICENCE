...
std::vector< Vec3 > bezierCurveByBernstein(std::vector< Vec3 > & TabControlPoint, int nbControlPoint, float nbU) {
    ...
    for(float t=0.0f; t<=1.0f; t+=1.0f/nbU) {
        ...
        for(int i=0; i<=nbControlPoint; i++) {
            float bernstein = (fact(nbControlPoint)/(fact(i)*fact(nbControlPoint-i)))*powf(t,i)*pow(1-t,nbControlPoint-i);
            point += Vec3(bernstein * TabControlPoint[i]);
        }
...
std::vector< Vec3 > bezierCurveByCasteljau(std::vector< Vec3 > TabControlPoint, float nbControlPoint, float nbU) {
    std::vector< Vec3 > tabU;
    for(float t=0.0f; t<=1.0f; t+=1.0f/nbU) {
        Vec3 point = Vec3( 0.0f, 0.0f, 0.0f );
        for(int j=0; j<=nbControlPoint; j++) {
            for(int i=0; i<=nbControlPoint-j; i++) {
                float bernstein = (fact(nbControlPoint-j)/(fact(i)*fact(nbControlPoint-j-i)))*powf(t,i)*pow(1-t,nbControlPoint-j-i);
                point += Vec3(bernstein * Vec3(powf(TabControlPoint[i][0],j), powf(TabControlPoint[i][1],j), powf(TabControlPoint[i][2],j)));
            }
            tabU.push_back(point);
        }
    }
...
void draw () {
    ...
    float nbU = 100.0f;
    /* Test courbe cubique Hermite
    Vec3 p0 = Vec3(0.0f,0.0f,0.0f);
    Vec3 v0 = Vec3(1.0f,1.0f,0.0f);
    Vec3 p1 = Vec3(2.0f,0.0f,0.0f);
    Vec3 v1 = Vec3(1.0f,-1.0f,0.0f);
    TabPointsOfCurve = cubicHermiteCurve(p0,p1,v0,v1,nbU);
    drawCurve(TabPointsOfCurve, nbU);
    */
    ...
    drawCurve(controlPoints, controlPoints.size());
}
