...
struct Scene{
    ...
    void render() {
        ...
        // Set the lights in the shader :
        {
            ...
            shaderProgramPtr->set (std::string("lightSources[0].position"), light.m_position);
            shaderProgramPtr->set (std::string("lightSources[0].color"), light.m_color);
            shaderProgramPtr->set (std::string("lightSources[0].intensity"), light.m_intensity);
            shaderProgramPtr->set (std::string("lightSources[0].isActive"), 1);

            Light & light2 = scene_lights[1];
            shaderProgramPtr->set (std::string("lightSources[1].position"), light2.m_position);
            shaderProgramPtr->set (std::string("lightSources[1].color"), light2.m_color);
            shaderProgramPtr->set (std::string("lightSources[1].intensity"), light2.m_intensity);
            shaderProgramPtr->set (std::string("lightSources[1].isActive"), 1);
        }
...
void initScene (const std::string & meshFilename) {
    ...
    {
        // for example:
        GLuint shadom0 = Texture::loadTextureFromFileToGPU("shadom_map_0.ppm");
        GLuint shadom1 = Texture::loadTextureFromFileToGPU("shadom_map_0.ppm");
    }
    ...
    // Setup lights :
    {
        ...
        {
            scene.scene_lights.resize( 2 );
            Light & newLight = scene.scene_lights[ 0 ];
            newLight.m_position = glm::vec3 (10., 10., -5.);
            newLight.m_color = glm::vec3(1.0, 1.0, 1.0);
            newLight.m_intensity = 0.75f;
            ...
            Light & newLight2 = scene.scene_lights[ 1 ];
            newLight2.m_position = glm::vec3 (-2*cos(app_timer), 2*sin(app_timer), 3.5);
