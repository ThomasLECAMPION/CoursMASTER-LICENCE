...
public:
    ...
    Vec3 apply_to_normalized_vector(Vec3 const &i_vector) {
        Vec3 result = m_vector_transformation * i_vector;
...
Vec3 project(Vec3 const &input_point, Plane const &i_plane) {
    Vec3 v = input_point - i_plane.point;
    float dist = v.dot(v, i_plane.normal);
    Vec3 result = input_point - dist*i_plane.normal;
...
Vec3 project(Vec3 const &input_point, Vec3 const &i_origin, Vec3 const &i_axis) {
    ...
    float dist = v.dot(v, i_axis);
    Vec3 result = input_point - dist*i_axis;
...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        for(unsigned int j=0; j<projection_on_basis.size(); j++) {
            float coord = projection_on_basis[j][i];
            moyenne += coord;
        }
        moyenne /= projection_on_basis.size();

        float i_variance=0.;
        for(unsigned int j=0; j<projection_on_basis.size(); j++) {
            float coord = projection_on_basis[j][i];
            i_variance += (coord - moyenne)*(coord - moyenne);
        }
        i_variance /= projection_on_basis.size();

        // Question 2.5: TODO Compléter
        variance[i] = i_variance;
    }
